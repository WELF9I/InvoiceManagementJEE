...
const CategoriesList: React.FC<{ categoriesData: DataItem[] }> = ({ categoriesData }) => {
  ...
  const Delete = async (id: number) => {
    try {
      const response = await fetch(`http://${apiUrl}:9090/categories/${id}`, {
        method: 'DELETE',
      });
      ...
      if (response.ok) {
        ...
        showToastWithGravity('Failed to delete Category');
      }
    } catch (error) {
  ...
  const updateCategoryInDatabase = async (id: number, name: string, categoryImage: string) => {
    try {
      const response = await fetch(`http://${apiUrl}:9090/categories/${id}`, {
        ...
        body: JSON.stringify({ categoryName: name, categoryImage: categoryImage }),
      });
  
      if (response.ok) {
        console.log('Category updated successfully in the database!');
        showToastWithGravity('Category updated successfully!');
      } else {
        const errorMessage = await response.text();
        console.error('Error updating Category:', errorMessage);
        showToastWithGravity('Failed to update Category');
      }
    } catch (error) {
      ...
      const updatedCategories = CategoriesData.map((category) =>
        category.idCat === editedCategory.idCat
          ? {
              ...
              categoryImage: logoToInsert || category.categoryImage,
            }
          : category
      );

      showToastWithGravity('Category updated');
      updateCategoryInDatabase(editedCategory.idCat, editedCategoryName, logoToInsert);
        ...
        CategoriesData.map((category: any, index: any) => (
          <View key={index} style={styles.cardContainer}>
            <View style={styles.cardHeader}>
              ...
              <Image source={{ uri: `data:image/png;base64,${category.categoryImage}` }} style={styles.SoftHardicons} />
            </View>
