    ...
    try {
      ...
      const fileUri = pathPdf.replace('file://', '');
        ...
        const newExpenseData = {
            amount: newAmount,
            dateExpense: concatenateDateTime(selectedDate, selectedTime),
            pdfFile: fileContent,
      ...
      const response = await fetch(`http://${apiUrl}:9090/expenses/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!response.ok) {
        ...
        console.log('Inserted expense:', insertedExpense); // Log the response here
      }
